	...
	<view class="content">
		<view class="order_details_1">
			<view class="order_details_7">
				<text decode="true"
					class="status">{{itemInfo.status == 1 ? '待捡货' : itemInfo.status == 2 ? '待发货' : itemInfo.status == 3 ? '已完成' : itemInfo.status == 4 ? '已取消' : '无状态'}}</text>
				<text decode="true" class="smallTitle">是否收款：{{itemInfo.pay_status == 1 ? '未收款' : '已收款'}}</text>
			</view>
			<view class="order_details_2">
				<view class="goods">
					<view v-for="(item, index) in itemInfo.goods_data" :key="index" class="row">
						<image :src="showImg(item.sku.default_image_url)" mode="scaleToFill" border="0" class="pic">
						</image>
						<view class="order_details_10">
							<text decode="true" class="good">{{item.sku.name}}</text>
							<text decode="true" class="num">数量：{{item.fixed_weight ? item.fixed_weight : item.count}}</text>
						</view>
						<text decode="true" class="money">价格：{{item.price}}</text>
					</view>
				</view>
				<view class="order_details_15">
					<text decode="true"
						class="order_details_16">共{{itemInfo.goods_data.length}}份小计￥{{itemInfo.total_amount}}</text>
				</view>
			</view>
			<view class="order_details_2">
