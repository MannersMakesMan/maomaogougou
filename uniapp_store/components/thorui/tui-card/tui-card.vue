	 ...
	 @longtap="longTap">
		<view class="tui-card-header" :class="{'tui-header-line':header.line}" :style="{background:header.bgcolor || '#fff'}">
			<view class="tui-header-left">
				...
				<text class="tui-header-title" :style="{fontSize:32 +'rpx',color:'#343434'}" v-if="title">{{title}}</text>
			</view>
			<view class="tui-header-right" :style="{fontSize:24+'rpx',color:'#b2b2b2'}" v-if="tag">
				{{tag}}
			</view>
		</view>
