...
class ParamUnittestSuite(unittest.TestSuite):
    ...
    def _get_param_unittest(self, module_name, test_case, test_method,params):
        ...
        for each in dir(module):
            if each.startswith(test_case_name+'_') and each.split('_')[-1].isdigit():
                ...
                super().addTest(new_test_case(test_method,params))

    def addTest(self, test_case: object, test_method: object,params) -> object:
        if not hasattr(test_case, '__is_parameter_test_case__'):
            super().addTest(test_case(test_method,params))
        else:
            self._get_param_unittest(test_case.__module_name__, test_case, test_method,params)
