...
from user_interface_test_code.models import CommonParams, TestAppModel, UiTestScene, TestCase, TestCaseData, UiSceneParams, \
...
from user_interface_test_code.serializers import UiTestParamsSerializerQuery, UiTestParamsSerializerAdd, \
    TestModelSerializerQuery, TestModelSerializerAdd, TestModelSelectSerializerQuery, TestSceneSerializerQuery, \
    TestSceneSerializerAdd, TestSceneParamsSerializer
from user_interface_test_code.tools import get_one_test_scene_data
                ...
                if scene_param_objs:
                    ...
                    for scene_param_obj in scene_param_objs:
                        ...
                        for case_step_obj in case_step_objs:
                            ...
                            param_value = param_dic[case_step_obj.step_desc] if param_dic.get(case_step_obj.step_desc, "") else ''
                            param_dic[step_desc] = param_value
                        all_scene_param.append({"id": scene_param_obj.id, "sort": scene_param_obj.sort, "param_dic": param_dic})
                    return_data = {"table_header": table_header, "param_info": all_scene_param}

                else:
                    # 无历史参数数据 根据测试用例生成第一条默认数据
                    # 测试场景下 单用例的 参数字典 以参数步骤作为键名
                    case_step_objs = TestCaseData.objects.filter(test_case_id=int(case_id)).order_by("sort")
                    param_dic = {}
                    for case_step_obj in case_step_objs:
                        if case_step_obj.is_need_value:
                            ...
                            continue
                        table_header.append(case_step_obj.step_desc)
                        param_dic[case_step_obj.step_desc] = value
                    if param_dic:
                        test_scene_data_obj = UiSceneParams(test_case_index_id=int(test_case_index_id), param_dic=str(param_dic), sort=1)
                        test_scene_data_obj.save()
                        return_data = {"table_header": table_header, "param_info": [{"id": test_scene_data_obj.id, "sort": 1, "param_dic": param_dic}]}
                    else:
                        return_data = {"table_header": table_header, "param_info": []}
    ...
    @swagger_auto_schema(
        ...
        request_body=openapi.Schema(
            ...
            properties={
                ...
                'param_dic': openapi.Schema(type=openapi.TYPE_OBJECT, description='参数字典', properties={
                }),
            })
    )
    def post(self, request):
