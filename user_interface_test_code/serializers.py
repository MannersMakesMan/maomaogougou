...
from user_interface_test_code.models import CommonParams, TestAppModel, TestCase, TestCaseData, UiTestScene, UiSceneParams
from user_interface_test_code.models import CommonParams, UiFunctions
...
class TestModelSerializerQuery(serializers.ModelSerializer):
    ...
    class Meta:
        model = TestAppModel
        fields = ('id', 'entry_name_id', 'model_name', 'fun_name', 'interface_name', 'dev_user', 'test_user', 'create_time', 'update_time', 'remark', 'update_user')


class TestModelSerializerAdd(serializers.ModelSerializer):
    entry_name_id = serializers.CharField()
    model_name = serializers.CharField(required=True, error_messages={'required': '模块名为空'})
    fun_name = serializers.CharField(required=True, error_messages={'required': '功能名为空'})
    interface_name = serializers.CharField(required=True, error_messages={'required': '界面名为空'})
    # interface_url = serializers.CharField(required=True, error_messages={'required': '测试地址为空'})
    dev_user = serializers.CharField()
    test_user = serializers.CharField()

    def validate_entry_name_id(self, entry_name_id):
        if not Project.objects.filter(id=entry_name_id):
            raise serializers.ValidationError('项目id错误！！！')
        return entry_name_id
    ...
    class Meta:
        model = TestAppModel
        fields = ('id', 'entry_name_id', 'model_name', 'fun_name', 'interface_name', 'dev_user', 'test_user', 'remark', 'update_user')
...
class TestCaseQuerySerializers(serializers.ModelSerializer):
    ...
    def get_test_app_model_id(self, obj):
        if obj.test_app_model_id:
            data = TestModelSerializerQuery(TestAppModel.objects.filter(id=obj.test_app_model_id).first()).data
            return {
                'id': data.get('id'),
                'model_name': data.get('model_name'),
                'fun_name': data.get('fun_name'),
...
class TestCaseDataSerializerQuery(serializers.ModelSerializer):
    ...
    step_desc = serializers.SerializerMethodField()
    ...
    def get_step_desc(self, obj):
        step_desc = obj.step_desc
        if "_sort" in step_desc:
            return step_desc.split("_sort")[-2]
        else:
            return step_desc
...
class TestCaseDataSerializerAdd(serializers.ModelSerializer):
    ...
    def validate_step_desc(self, step_desc):
        return "{}_sort{}".format(step_desc, self.initial_data.get("sort"))
