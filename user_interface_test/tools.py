...
from user_interface_test.models import TestCaseData, UiTestScene, CommonParams, UiSceneTestCaseIndex, UiSceneParams, \
    TestCase
from user_interface_test.serializers import TestCaseDataSerializerQuery
...
def get_one_test_case_data(test_case_ids, environment_ip, explicit_wait_timeout, explicit_wait_poll_time, implicitly_wait_timeout, remote_ip):
    ...
    for test_case_id in test_case_ids:
        ...
        for step_index, step_data in enumerate(step_data_obj_list.data):
            item = {}
            if step_index == 0:
                # 用例数据拼接 用例切换后 增加用例切换方法
                switch_case_item = {'function': 'switch_case',
                                    'value': TestCase.objects.get(id=test_case_id).case_name}
                step_list.append(switch_case_item)
...
def get_one_test_scene_data(test_scene_id, environment_ip, remote_ip):
    ...
    test_scene_desc = UiTestScene.objects.get(id=test_scene_id).scene_desc
        ...
        if not scene_param_objs:
            ...
            for case_step_obj in case_step_objs:
                if case_step_obj.is_need_value:
                    ...
                    continue
                param_dic[str(case_step_obj.id)] = value
            if param_dic:
    ...
    for index in range(0, scene_exe_time):
        ...
        for case_index_obj in case_index_objs:
            ...
            for step_index, step_obj in enumerate(step_objs):
                item = {}
                if step_index == 0:
                    # 用例数据拼接 用例切换后 增加用例切换方法
                    switch_case_item = {'function': 'switch_case',
                                        'value': TestCase.objects.get(id=case_index_obj.test_case_id).case_name}
                    step_list["steps_data"].append(switch_case_item)

                id = step_obj.id
                if step_obj.is_need_assert:
                    try:
                        item['assert_value'] = param_dic[str(id)]
                        if not item['assert_value']:
                            continue
                    except Exception as _:
                ...
                if step_obj.mysql_info_id:
                    ...
                    item['mysql_info'] = {}
                item['step_desc'] = step_obj.step_desc
    ...
    execution_test_case_data = {
        ...
        'explicit_wait_poll_time': explicit_wait_poll_time,  # 显式等待轮询时间
        'implicitly_wait_timeout': implicitly_wait_timeout,  # 隐式等待时间
        'test_scene_desc': test_scene_desc,
        'test_case_data': test_case_data
    }
    return execution_test_case_data
