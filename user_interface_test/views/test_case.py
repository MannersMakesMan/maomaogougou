...
from user_interface_test.models import CommonParams, TestAppModel, TestCase, UiFunctions, TestCaseData, UiTestScene, \
...
from user_interface_test.tools import get_one_test_case_data
...
class TestCaseTreeQueryView(APIView):
    authentication_classes = ()
    permission_classes = []

    @swagger_auto_schema(
        operation_summary='测试用例树形结构全量查询 项目-模块-参数-用例 用于拼接场景/筛选用例下拉框',
        manual_parameters=[
            ...
            for project_obj in project_objs:
                ...
                for model_obj in TestAppModel.objects.filter(super_id=project_obj.id, model_level=1):
                    ...
                    for page_obj in TestAppModel.objects.filter(super_id=model_obj.id, model_level=2):
                        test_case_ls = []
                        page_dic = {
                            "model_name": page_obj.model_name,
                            "id": page_obj.id,
                            "children": test_case_ls
                        }
                        page_dic_ls.append(page_dic)
                        for test_case_obj in TestCase.objects.filter(test_app_model_id=page_obj.id):
                            test_case_dic = {
                                "model_name": test_case_obj.case_name,
                                "id": test_case_obj.id
                            }
                            test_case_ls.append(test_case_dic)
...
class TestCaseQueryView(APIView):
    ...
    @swagger_auto_schema(
        ...
        manual_parameters=[
            ...
            openapi.Parameter(name='test_app_model_id', in_=openapi.IN_QUERY, description='界面id',
                              type=openapi.TYPE_STRING),
    ...
    def get(self, request):
        try:
            ...
            total, obm = api_list_query(data, page, page_size, TestCase, 'id',
                                        ['test_app_model_id', 'case_number', 'case_name'],
                                        accurate_ls=['test_app_model_id'])
            try:
