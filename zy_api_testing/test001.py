...
# # import ImageEnhance,ImageFilter from PIL.ImageEnhance import
# import PIL.ImageEnhance as ImageEnhance ,PIL.ImageFilter as ImageFilter
# import sys
...
def main():
  ...
  for file in os.listdir(filedir):
    if fnmatch(file, '*.png'):
      ...
      # 识别验证码
      cutting_img_num = 0
      for file in os.listdir('./image_ls'):
        str_img = ''
      #   if fnmatch(file, '%s-cutting-*.png' % img_name.split('.')[0]):
      #     cutting_img_num += 1
      # for i in range(cutting_img_num):
        try:
          # file = './image_ls/%s-cutting-%s.png' % (img_name.split('.')[0], i)
          file = './image_ls/test.png'
          # 识别验证码
